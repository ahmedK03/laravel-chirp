        ...
        <section class="mt-9 bg-white shadow-sm rounded-lg divide-y px-5 py-6">
            @foreach ($chirps as $chirp)
                <div class="flex space-x-3 mb-4">
                    ...
                    <div class="flex-1">
                        <div class="inline-flex justify-start items-center italic leading-tight">

                                <span class="text-gray-600">{{ $chirp->user->name }}</span>
                                <small class="ml-2 text-sm text-gray-400">{{$chirp->created_at->format('j M Y, g:i a')}}</small>

                        </div>
                        <p class="text-xl mt-3 text-gray-800">{{$chirp->message}}</p>
                    </div>
                </div>
            @endforeach
        </section>
